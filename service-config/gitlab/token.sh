...
body_header=$(curl -Ls -b cookies.txt "${gitlab_host}/profile/personal_access_tokens" \
...
echo "private_token: $personal_access_token"
#curl -sSfL --header "PRIVATE-TOKEN: $personal_access_token" $gitlab_host/api/v4/version | python -mjson.tool
